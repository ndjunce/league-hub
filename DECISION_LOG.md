...
## 2026-09-23 — Playoff Races + Stakes all-week tabs — blast radius, BUILDING
Freeze before: good-hub-pre-races -> 9bc9a05 (pushed). Additive only; commit ndjunce/noreply. Complements the shipped Playoff Picture + Weekly Stakes tabs — REUSES their helpers (oddsSwing, seedMapIfWin, remainingMatchups, boundsFor, importanceBand, standings, weekGames), does NOT rebuild or edit them.

**PART 1 — Playoff Races (two sections inside the existing 🏆 Playoff Picture tab):** (a) #1-SEED / FIRST-ROUND-BYE race — contenders whose win-out ceiling (w+gamesLeft) ≥ the leader's lose-out floor; current leader; games-back (leader.w - t.w); what each needs; CLINCHED #1 when a team's floor (w) > every other team's ceiling (w_other+gamesLeft). (b) GENERAL PLAYOFF-SPOT race — for the playoffTeams cutoff: IN (seed≤cutoff), BUBBLE (cut line reachable both ways via boundsFor: bestSeed≤cutoff<worstSeed), CHASING (out now but ceiling still reaches the cut team's floor); games-back from the cut line; magic/elimination number where computable. Same win-out/lose-out honesty as paintPlayoffs; only mark CLINCHED/ELIM when certain; say "too early" plainly when everyone's alive.

**PART 2 — Stakes all-week tabs:** add STAKES_WEEK state + a wtabs-style week row (1..regWeeks) to paintStakes, default D.week. Generalize the matchup source from currentWeekMatchups (current only) to a week param. HONESTY: current week = exact enumerated odds as today; future weeks = show schedule (who-plays-who) + directional/bounds estimate LABELED "projected — gets exact as it approaches" (never a fake %); past weeks = optional known-result retrospective. Reuses the shipped odds math unchanged.

**BLAST RADIUS (additive):** ADDS seedRaceData(D) + spotRaceData(D) pure fns; two render blocks appended to paintPlayoffs's s array before el.innerHTML (existing blocks untouched); STAKES_WEEK var + week-tab render + wiring in paintStakes; .race-* + reused .wtab CSS. Does NOT touch: ESPN proxy, standings(D), weekGames(D), schedule/roster parsing, paintInsights Weekly Results, paintTradeBlock, or the shipped oddsSwing/seedMapIfWin/boundsFor/importanceBand math (reused, not edited). Different repo from the frozen engine/CAN AM board. Verify then log WORKS/OPEN.


## 2026-09-23 — Playoff Races + Stakes all-week tabs SHIPPED — WORKS
Freeze before: good-hub-pre-races -> 9bc9a05 (pushed). Additive only; commit ndjunce/noreply. REUSED the shipped Stakes/Playoff helpers (standings, boundsFor, seedMapIfWin, oddsSwing, importanceBand, weekGames) — did NOT rebuild or edit them.

**PART 1 — Playoff Races (two sections appended inside the existing 🏆 Playoff Picture tab, existing blocks untouched):** new pure fns seedRaceData(D) + spotRaceData(D). (a) 🥇 #1-SEED/BYE race: contenders = teams whose win-out ceiling (w+gamesLeft) ≥ the leader's lose-out floor; shows leader, games-back (leader.w−t.w), what each needs; CLINCHED #1 only when a team's floor > every other team's ceiling. (b) 🎟️ PLAYOFF-SPOT race: IN (seed≤cutoff) / BUBBLE (cut line reachable both ways via boundsFor best≤cutoff<worst) / CHASING (out but ceiling still reaches the cut floor) / OUT (can't reach even winning out); games-back from the cut line; magic number (wins that guarantee — floor beats best chaser ceiling) and elim number (losses that end it — ceiling can't reach cut floor), both blank until certain; cut-line divider row.

**PART 2 — Stakes all-week tabs:** added STAKES_WEEK state + a wtabs-style week row (1..regWeeks) to paintStakes (default D.week), generalized the matchup source to weekMatchups(D,wk). HONESTY by distance: current week = exact enumerated odds (unchanged from the shipped math); future weeks = schedule + directional seed-swing + best/worst bounds, LABELED "hasn't been played … projected … gets exact as it approaches," NEVER a fabricated %; past weeks = actual result + "in the books" retrospective (cut-line-swing note in hindsight), no hypothetical Win/Lose chips.

**Verified (node harness, 8-team/6-week/wk4 synthetic + hand math):** JS parses clean. seedRaceData: leader T1, correctly NOT clinched (T2 ceiling 5 ≥ T1 floor 3), all 8 contenders (ceilings ≥ leader floor 3), games-back 0/1/1/1/2/2/2/3 — matches by hand. spotRaceData: cut team T4@2W, early-season everyone BUBBLE (spans the line both ways), IN teams get magic numbers / OUT teams get elim numbers, none falsely clinched/eliminated — matches by hand. Week tabs: wk3 past label + result badge + NO odds % + NO hypothetical chips; wk4 current shows exact odds %; wk5 future shows projected label + bounds + NO fabricated %; default week when null → D.week (4). paintPlayoffs still renders the existing Playoff Picture + Standings & Status blocks (untouched) plus the two new race sections. No proxy/standings/schedule/Trade Block/shipped-Stakes-math edits. Mobile: race rows + chips use flex-wrap + name ellipsis, race-need reflows below @<520px.


## 2026-09-23 — Weekly Stakes importance-band compression gate — WORKS
Freeze before: good-hub-pre-bands -> 9aecf1a (pushed). Additive; commit ndjunce/noreply. Reuses standings(D)/importanceBand — did NOT rewrite oddsSwing or touch the just-shipped Races/week-tab code.

**Problem:** at Week 3 the whole field is within ~2 wins, so every undecided game is genuinely high-leverage — but the bands were splitting tiny playoff-odds-swing differences (within noise this early) and flagging a lone outlier as MEANINGFUL (e.g. Thor vs Zach) while everything else read PIVOTAL.

**Fix:** gate the fine-grained split on STANDINGS COMPRESSION, not odds-swing hairs. New pure fn standingsCompression(D): win-spread among playoff-relevant teams (those whose win-out ceiling w+gamesLeft ≥ the current cut-line team's wins) = max(w)−min(w). tight = spread ≤ 2. Extended importanceBand(score, crossesLine, tight) with an OPTIONAL 3rd arg — when tight, force PIVOTAL for every undecided game; otherwise the existing cross/0.20/0.05 thresholds are unchanged (2-arg callers behave identically → Races code untouched). paintStakes computes comp = standingsCompression(D) once, tightBands = comp.tight && !isPast, passes it into the band call, and shows an honest framing note: "Standings are tightly bunched (playoff-relevant teams within N games) — a single result can reshuffle the seeding, so every game matters this week. The PIVOTAL/MEANINGFUL/LOW split kicks in once teams separate." Matches the Hub's "gets sharper each week" discipline.

**Verified (node harness + hand math):** JS parses clean. CASE A (8 teams bunched 2-1/2-1/2-1/1-2…, spread 2, tight) → all 4 current-week games read PIVOTAL (stk-imp pivotal:4, meaningful:0, low:0), framing note present — the lone-MEANINGFUL outlier is gone. CASE B (separated 8-1…1-8, spread 7, not tight) → bands differentiate (pivotal:0, meaningful:1, low:3), no framing note. importanceBand unit: tight=true+low score+no cross → PIVOTAL; 2-arg (Races) score .08 → MEANINGFUL, .01 → low, cross → PIVOTAL (all unchanged). Only one importanceBand call site (paintStakes L950); Races don't call it. Existing tabs unchanged; framing note is a single .cap line (no overflow @390px).


## 2026-09-23 — Stakes/Playoff-Spot Race three-issue fix — WORKS
Freeze before: good-hub-pre-racefix -> 48cd5cc (pushed). Additive; commit ndjunce/noreply. Reused standings/oddsSwing/boundsFor/spotRaceData/seedRaceData — no proxy/parsing changes.

**Issue 1 — "Seeding" and "Both" rendered identically (BUG).** Root cause: in the odds chip branch, the honest win-out/lose-out BOUNDS fallback (for the not-enumerable / future-week case) was gated on `else if(STAKES_MODE==="odds")` — so in "both" mode, when odds weren't enumerable (early season), the odds side emitted NOTHING and "both" collapsed to the seeding chips only. Fix: changed that to a plain `else` so BOTH and odds both fall back to bounds when there's no exact %. Now the three modes are distinct: seeding = seed chips only; odds = odds%/bounds only; both = seed chips + odds%/bounds. Verified (12-team wk3): seeding {seed:24,bounds:0}, odds {seed:0,bounds:12}, both {seed:24,bounds:12}; seeding!==both.

**Issue 2 — PF tiebreak when level on games-back.** standings(D) breaks ties W-then-PF, so anywhere games-back is shown we now surface PF on a tie. Playoff-Spot rows: pfTie flag (same wins as an adjacent row) → " · N pf breaks the tie" instead of plain pf. #1-Seed race: contenders sharing a games-back value get the same "pf breaks the tie" note. PF is still always shown in the record cell regardless.

**Issue 3 — Playoff-Spot race was uninformative (everyone BUBBLE, "seed range 1–12").** With 12 games left the theoretical win-out/lose-out bounds span the league, so a 2-0 and an 0-2 team read identically. Fix leans on CURRENT position, not the extremes: (a) rows already lead with current seed (sorted W then PF) so ordering carries info; (b) replaced the "seed range 1–12" headline with the REAL signal — games from the cut line by today's record (in-line: "+N games above the line" cushion over the first team out; out-line: "N games back") + PF; (c) BAND now = current standing: BUBBLE reserved for the seeds straddling the line (seed in [cutoff-1 .. cutoff+2]), clearly-above-by-seed = IN, clearly-below = CHASING; true CLINCHED (canBeIn && !canBeOut) / ELIMINATED (!canBeIn) unchanged; (d) the honest wide bounds are suppressed as a headline early (boundsWide flag = range spans ≥ size-1) and the seed range only shows once it narrows, plus magic/elim only when certain — matching the "gets sharper each week" discipline.

**Verified (node harness + hand math):** JS parses clean. Early bunched 12-team wk3: Seth 2-0 → IN seed1 "+1 game above the line", Bobby 0-2 → CHASING seed12 "1 game back" (no longer identical); no "seed range 1–12" headline; cut-line divider + #1-seed race + existing Standings & Status intact; "pf breaks the tie" present. Late separated wk12/13: bands in,in,in,in,bubble,bubble,bubble,out,out,out,out,out (proper tiers); wide-bounds suppression lifts so seed range shows; magic + elim surface. Three Stakes modes distinct. Existing tabs unchanged; race-need reflows @<520px so no 390px overflow.
